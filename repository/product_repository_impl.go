...
func (repository *ProductRepositoryImpl) FindById(ctx context.Context, productId uint64) (domain.Product, error) {
	...
	err := repository.db.WithContext(ctx).Preload("Category").First(&product, "id = ?", productId).Error
...
func (repository *ProductRepositoryImpl) FindAll(ctx context.Context) ([]domain.Product, error) {
	...
	err := repository.db.WithContext(ctx).Preload("Category").Find(&products).Error
	return products, err
}

// FindByCategory - Get products by Category ID
func (repository *ProductRepositoryImpl) FindByCategory(ctx context.Context, categoryId uint64) ([]domain.Product, error) {
	var products []domain.Product
	err := repository.db.WithContext(ctx).Where("category_id = ?", categoryId).Preload("Category").Find(&products).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return products, errors.New("no products found for this category")
	}
	return products, err
}
